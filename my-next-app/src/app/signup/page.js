    ...
    <div className={styles.main}>
      <div className={styles.container}>
        <div className={styles.form}>
          <form className={styles.formContent} style={{ display: display }} onSubmit={(e)=>{
            e.preventDefault();
            if (form.pass !== form.confirmpass) {
              setsame(<p style={{ color: 'red', margin: '0' }}>Both passwords should be same to sign up</p>);
              return;
            }
            setsubmit(curr => curr + 1)
          }}>
            <h2 className={styles.signupHeading}>Sign Up</h2>
            <div className={styles.inputWrapper}>
              <MdPerson className={styles.inputIcon} />
              <input className={styles.input} placeholder='Full Name' required value={form.name} onChange={e => (setform(curr => ({ ...curr, name: e.target.value })))} />
            </div>
            <div className={styles.inputWrapper}>
              <MdEmail className={styles.inputIcon} />
              <input className={styles.input} placeholder='Email' required value={form.email} onChange={e => setform(curr => ({ ...curr, email: e.target.value }))} />
            </div>
            <div className={styles.inputWrapper}>
